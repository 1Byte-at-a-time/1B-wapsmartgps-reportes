	...
	public function massAction($ids, $comingFrom)
	{
		...
		switch (true) {
			...
			case str_contains($this->dataType->model_name, 'Center'):
				...
				break;
				break;
			case str_contains($this->dataType->model_name, 'CostosAccidente'):
				$data = app($this->dataType->model_name)->all();
				if ($profile->role_id != 1) {
					
					
					$data = $profile->role_id == 1 ? app($this->dataType->model_name)->all() : DB::select(
					'select * from costos_accidentes where center_id=?;',
					[$profile->center_id],);
				}
				$displayColumns = DB::select("SELECT display_name FROM data_rows where data_type_id=?;", [19], );
				$idToSearch = 19;
				break;
			case str_contains($this->dataType->model_name, 'MantenimientosCorrectivo'):
				$data = app($this->dataType->model_name)->all();
				if ($profile->role_id != 1) {
					
					
					$data = $profile->role_id == 1 ? app($this->dataType->model_name)->all() : DB::select(
					'select * from mantenimientos_correctivos where center_id=?;',
					[$profile->center_id],);
				}
				$displayColumns = DB::select("SELECT display_name FROM data_rows where data_type_id=?;", [21], );
				$idToSearch = 21;
				break;
			case str_contains($this->dataType->model_name, 'Report'):
				...
				break;
			case str_contains($this->dataType->model_name, 'Models\Breporte'):
				$data = $profile->role_id == 1 ? app($this->dataType->model_name)->all() : DB::select(
					'SELECT DISTINCT r.created_at AS fecha_hora_reporte, v.license AS placa_vehiculo, r.*, u.*
        FROM breportes AS r
        INNER JOIN users AS u ON r.firma_digital = u.id
        INNER JOIN vehicles AS v ON r.license_id = v.id
        WHERE u.center_id = ?;
',
					[$profile->center_id],
				);
				...
				break;
			case str_contains($this->dataType->model_name, 'Models\Creporte'):
				$data = $profile->role_id == 1 ? app($this->dataType->model_name)->all() : DB::select(
					'SELECT DISTINCT r.created_at AS fecha_hora_reporte, v.license AS placa_vehiculo, r.*, u.*
        FROM creportes AS r
        INNER JOIN users AS u ON r.firma_digital = u.id
...
',
				...
				break;
			case str_contains($this->dataType->model_name, 'Models\Areporte'):
				$data = $profile->role_id == 1 ? app($this->dataType->model_name)->all() : DB::select(
					'SELECT DISTINCT r.created_at AS fecha_hora_reporte, v.license AS placa_vehiculo, r.*, u.*
        FROM areportes AS r
        INNER JOIN users AS u ON r.firma_digital = u.id
...
',
		...
		if (!str_contains($this->dataType->model_name, 'Report') && $idToSearch== 6) {
		...
		$callback = function () use ($data, $columns, $displayColumns) {
			...
			foreach ($data as $data_col) {
				if (str_contains($this->dataType->model_name, 'Report') || $idToSearch = 17 || $idToSearch = 16 || $idToSearch = 15) {
